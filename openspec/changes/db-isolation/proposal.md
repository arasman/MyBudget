...
- [ ] `dotnet test tests/MyBudget.Integration.Tests/` passes reading conn string from config
- [ ] E2E tests run against `mybudget_e2e`, verified by checking DB name in test output
- [ ] Dev database (`mybudget`) has zero test-generated rows after full test suite run
- [ ] `POST /api/test/reset` returns 404 when API runs in Development mode
- [ ] `POST /api/test/reset` returns 200 and wipes+migrates when API runs in E2E mode
- [ ] `openspec/config.yaml` integration note corrected to mention Postgres
