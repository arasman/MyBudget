...
- **Reorder**: drag-and-drop via HTML5 drag API or lightweight library (e.g., `@vueuse/integrations` sortable). Calls existing `PUT .../order` endpoints.
...
|------|--------|-------------|
| `frontend/src/App.vue` | Modified | Simplified to `<RouterView>` only (already is, but router nesting changes) |
...
| `frontend/src/views/HomeView.vue` | Modified | Removes inline navbar |
...
All changes are additive frontend files + one backend line. Rollback = revert the merge commit(s). Layout routes can be unwound by restoring `router/index.ts` to pre-change state. No database migrations involved.
...
- [ ] Authenticated users see AppLayout navbar with budget switcher on all protected routes
- [ ] Single-membership users auto-redirect to their budget's structure view
- [ ] Full CRUD for Cycles, Periods, CategoryGroups, Categories, BudgetLines via UI
- [ ] Drag-and-drop reorder works for CategoryGroups and Categories
- [ ] Role-based UI gating: admin-only actions hidden for operator/read-only users
- [ ] Empty states show guided setup prompts
- [ ] All UI strings available in EN and ES
- [ ] vue-i18n `@` bug fixed in 4 locations
- [ ] Scalar API explorer accessible at `/scalar/v1`
- [ ] Login/Register views render correctly under PublicLayout
