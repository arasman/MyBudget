...
**Post-verify fixup (still Unit 5 scope, no new task numbers):**
- Verify flagged design decision #7 (reduced motion) as untested: `main.css`'s `@media (prefers-reduced-motion: reduce) { .showcase-zoom-card { transition: none } }` rule existed, but no Vitest or Playwright test exercised it, and `proposal.md`'s Success Criteria checkbox 5 cited the CSS rule as its own evidence (circular). Added a Playwright case to `Project/frontend/e2e/landing/landing.spec.ts` — `page.emulateMedia({ reducedMotion: 'reduce' })` before navigation, clicks a tile active (same zero-delay trigger the "no horizontal overflow while a tile is active" case already uses), and asserts `getComputedStyle(tile).transitionDuration === '0s'` and `transitionProperty === 'none'`. Proved the test is meaningful (not tautological): temporarily commented out the `prefers-reduced-motion` block in `main.css`, confirmed the new test goes RED (`transitionDuration` read the real `0.18s, 0.18s, 0.18s` durations instead of `0s`), restored the block, confirmed GREEN. Full `landing.spec.ts` re-run (`--project=chromium`, 12/12 pass) and `pnpm test` (87/87 files, 728/728 tests) both green afterward. `proposal.md`'s Success Criteria checkbox 5 updated to cite this test instead of the CSS rule alone.
