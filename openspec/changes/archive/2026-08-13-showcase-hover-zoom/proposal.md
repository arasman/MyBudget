...
- [x] With `prefers-reduced-motion: reduce`, the enlarged state still appears but without the scale/opacity animation. (E2E: `e2e/landing/landing.spec.ts` "respects prefers-reduced-motion: reduce by disabling the enlarge transition entirely" — `page.emulateMedia({ reducedMotion: 'reduce' })`, clicks a tile active, asserts `getComputedStyle(tile).transitionDuration === '0s'` and `transitionProperty === 'none'`; proven meaningful by a RED check with the `main.css` rule temporarily disabled, where the same assertion observed the real `0.18s, 0.18s, 0.18s` durations instead)
