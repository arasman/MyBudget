...
The forward-compat no-op in all four restore handlers (RST-2, RST-3, RST-4, RST-5) MUST be activated to implement REQ-EXEC-CASCADE-2.

#### Scenario: Previously no-op now restores children

- GIVEN any restore handler called with includeExecutionRecords=true
- WHEN handler processes the request
- THEN soft-deleted child ExecutionRecords are restored (was previously a no-op)
