...
type LineType = 'Expense' | 'LongTermSavings' | 'PreventiveSavings'
...
interface PeriodSummary {
  id: string; name: string; startDate: DateString
  endDate: DateString; status: string; periodNumber: number
}
...
budgetStructure:
  ...
  budgetLines:
    ...
    types: expense, longTermSavings, preventiveSavings
    empty: title, description, action
  common:
...
**Decision needed before apply**: No — chain order and branch strategy confirmed.

## Known Decisions

- `vue-draggable-plus` selected as D&D library (ADR-BSUI-01)
- PR chain targets feature branch `feat/budget-structure-ui` with child PRs
- Scalar API reference implemented (originally deferred in design)
- Inline editing provided for all entities (user-requested enhancement beyond spec)
- LineType values: Expense, LongTermSavings, PreventiveSavings (backend-correct; design doc should be updated)
