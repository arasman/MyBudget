...
The frontend MUST be located at `Project/frontend/`. The `src/` directory MUST contain the following subdirectories: `components/`, `stores/`, `router/`, `i18n/`, `api/`, `views/`, `types/`. No source files MAY be placed directly in `src/` except `main.ts` and `App.vue`.
...
- THEN all seven subdirectories (`components/`, `stores/`, `router/`, `i18n/`, `api/`, `views/`, `types/`) are present
...
vue-router MUST be configured with `createWebHistory`. Two placeholder routes MUST exist: `/login` → `LoginView.vue` and `/` (home) → `HomeView.vue`. Both views MUST render a minimal HTML structure sufficient to confirm the route is active.

#### Scenario: Root path renders HomeView

- GIVEN the app is loaded at `/`
- WHEN the router resolves the path
- THEN `HomeView.vue` content is rendered in the `<RouterView>`

#### Scenario: /login path renders LoginView

- GIVEN the app is loaded at `/login`
- WHEN the router resolves the path
- THEN `LoginView.vue` content is rendered in the `<RouterView>`
