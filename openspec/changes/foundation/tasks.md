...
### 1.1 Initialize git repository and create first commit on main
...
### 1.2 Create and checkout feature/foundation branch
...
### 2.1 Create solution file and src/tests directory structure
...
### 2.2 Create MyBudget.Features class library project
...
### 2.3 Create MyBudget.Api web host project
...
### 2.4 Create MyBudget.Gateway YARP proxy project
...
### 2.5 Create MyBudget.Features.Tests stub project
...
### 2.6 Create MyBudget.Integration.Tests stub project
...
### 3.1 Add NuGet packages to MyBudget.Features
...
### 3.2 Add NuGet packages to MyBudget.Api
...
### 3.3 Add NuGet packages to MyBudget.Gateway
...
### 3.4 Add NuGet packages to test projects
...
### 3.5 Verify full solution builds after all packages added
...
### 4.4 Create Caching abstractions (ICacheable, ICacheService, NullCacheService)
...
### 4.5 Create Email subtree (IEmailSender, EmailMessage, EmailChannel, EmailBackgroundService)
...
### 4.6 Create Persistence types (AppDbContext, ConnectionFactory)
...
### 4.7 Create SliceActivitySource
...
### 4.8 Verify full SharedKernel builds cleanly
...
### 5.1 Create ValidationBehaviour
...
### 5.4 Create ServiceCollectionExtensions (AddFeatures)
...
### 5.5 Create EndpointExtensions (MapAllSliceEndpoints)
...
### 6.1 Create CorrelationIdMiddleware
...
### 6.2 Create ExceptionMiddleware
...
### 6.3 Write full Program.cs for MyBudget.Api
...
### 6.4 Configure appsettings.json and initialize User Secrets
...
### 7.1 Generate InitialCreate migration
...
### 8.1 Wire YARP in MyBudget.Gateway/Program.cs
...
### 8.2 Configure YARP routes in appsettings.json
...
### 10.1 Scaffold Vite + Vue 3 + TypeScript project
...
### 10.3 Configure Tailwind v4 CSS-only (no tailwind.config.ts)
...
### 10.4 Configure path alias (@/) in vite.config.ts and tsconfig
...
### 10.5 Configure ESLint flat config
...
### 10.8 Create src/ directory skeleton
...
### 11.1 Create i18n locale files and initialization
...
### 11.2 Create Axios client singleton
...
### 11.3 Create Pinia stores (auth.store.ts, locale.store.ts)
...
### 11.4 Create placeholder views (LoginView.vue, HomeView.vue)
...
### 11.6 Create LanguageSwitcher component and wire main.ts + App.vue
