...
import type { BudgetLineResponse, DateString } from '@/features/budget-structure/types'
...
function makePeriod(startDate: string): PeriodSummary {
  return {
    ...
    endDate: startDate as DateString,
    isClosed: false,
  }
}
...
describe('isLineActiveForPeriod (REQ-BL-MATRIX-1, AD-5)', () => {
  it('perpetual line covers any period', () => {
    const line = makeLine({ startDate: '2025-01-01' as DateString, endDate: null })
  ...
  it('line starting ON period start covers period', () => {
    const line = makeLine({ startDate: '2025-06-01' as DateString, endDate: null })
    const period = makePeriod('2025-06-01')
    expect(isLineActiveForPeriod(line, period)).toBe(true)
  })
  ...
  it('line with endDate before period start does NOT cover period', () => {
    const line = makeLine({ startDate: '2025-01-01' as DateString, endDate: '2025-02-28' as DateString })
    const period = makePeriod('2025-03-01')
    expect(isLineActiveForPeriod(line, period)).toBe(false)
  })

  it('line with endDate ON period start covers period', () => {
    const line = makeLine({ startDate: '2025-01-01' as DateString, endDate: '2025-03-01' as DateString })
    const period = makePeriod('2025-03-01')
    expect(isLineActiveForPeriod(line, period)).toBe(true)
  })

  it('line with endDate after period start covers period', () => {
    const line = makeLine({ startDate: '2025-01-01' as DateString, endDate: '2025-12-31' as DateString })
    const period = makePeriod('2025-06-01')
    expect(isLineActiveForPeriod(line, period)).toBe(true)
  })
})
